...
export const updateUserValidation = [
  ...
  body('isAdmin').optional().custom((value) => {
...
export const updateUser = async (req,res,next)=>{
  try {
    ...
    const { password: _, otp: __, otpExpires: ___, refreshToken: ____, ...userResponse } = updatedUser.toObject();
...
export const getUser = async (req,res,next)=>{
  try {
    ...
    // SECURITY: Remove sensitive information from response
    const { password, otp, otpExpires, refreshToken, ...userResponse } = user.toObject();
    res.status(200).json(userResponse);
  } catch (err) {
    next(err);
  }
...
export const saveItem = async (req, res, next) => {
  try {
    ...
    // SECURITY: Remove sensitive information from response
    const { password, otp, otpExpires, refreshToken, ...userResponse } = updateUser.toObject();
