...
export const register = async (req, res, next) => {
  ...
  try {
    ...
    // Send OTP email using optimized service
    const emailResult = await sendOTPEmail(req.body.email, req.body.name, otpCode);
...
export const registerVerify = async (req, res, next) => {
  try {
    ...
    const { password: _, otp: __, otpExpires: ___, ...userResponse } = doc.toObject();
    res.status(200).json({ 
...
export const login = async (req, res, next) => {
  try {
    ...
    const { password, otp, otpExpires, refreshToken: _, ...otherDetails } = user._doc;
    ...
    res
      ...
      .json({ 
        ...
        user: otherDetails,
        access_token: accessToken, // SECURITY: Also send token in response for client storage
        refresh_token: refreshToken, // SECURITY: Also send refresh token for client storage
  ...
  try {
    ...
    res
      ...
      .json({ 
        ...
        access_token: newAccessToken, // SECURITY: Also send token in response for client storage
        expiresIn: 15 * 60
      });
  } catch (err) {
