...
const Notes = () => {
  ...
  const fetchNotes = async () => {
    ...
    const { data } = await axios.get(process.env.REACT_APP_PYQ);
    setData(data);
  };
  useEffect(() => {
    fetchNotes();
                    ...
                    >
                      <Link to={material.materialLink} target="_blank">
                        ...
                        {hoveredIndex === ind && material.desc && (
                          <motion.div
                            className="absolute bg-white text-black drop-shadow-md border-x-4 border-b-4 border-[#22a39f99] p-4  h-[160px] rounded-b-2xl bottom-0 left-0 right-0"
                    ...
                    .map((material, ind) => (
                      ...
                      >
                        <Link to={material.materialLink} target="_blank">
                          ...
                          {hoveredIndex === ind && material.desc && (
                            <motion.div
                              className="absolute bg-white text-black drop-shadow-md border-x-4 border-b-4 border-[#22a39f99] h-[160px] rounded-b-2xl  p-4 top-bottom left-0 right-0"
                              initial={{ x: -90, opacity: 0 }}
                              animate={{ x: 0, opacity: 0.8 }}
                            >
