...
const NavbarHead = () => {
  ...
  return (
    <div className="z-10 drop-shadow-sm w-full fixed top-0 left-0">
      <div className="md:flex md:items-center justify-between bg-white py-4 md:py-0 md:pl-10 md:pr-0 px-7">
        <Link
          to="/"
          onClick={() => {
            setOpen(!open);
          }}
          className="text-3xl inline text-indigo-600 mr-1 pt-2 font-bold cursor-pointer md:flex items-center font-[Poppins]"
        >
        ...
        <ul
          ...
          } bg-white  md:bg-blue-400 md:pl-16 pl-0 md:rounded-bl-full rounded-none h-auto md:h-16 md:flex md:items-center md:pb-0 pb-12 absolute md:static  w-full left-0 md:w-auto transition-all duration-500 ease-in`}
        >
          {Links.map((link) => (
            <li
              key={link.name}
